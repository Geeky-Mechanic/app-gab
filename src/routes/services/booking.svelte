...
const between = (newBook) => {
    ...
    return data.some(obj => {
        const begDate = new Date(obj.begHour);
        const endDate = new Date(obj.endHour);
...
const validateDate = () => {
    ...
    fullDate = new Date(year, month, day, hour, 0, 0);
    if(!between(fullDate) && !tooEarly(fullDate) && !tooLate(fullDate)){
        bookData.date = new Date(fullDate);
    }
    if(between(fullDate)){
        fullDate = "";
        errors.between = true;
...
$: freeHours = allHours.filter((hour)=> {
        ...
        const year = bookingDate?.getFullYear();
        const day = bookingDate?.getDate();
        const fDate = new Date(year, month, day, hour, 0, 0);
...
const handleClick = () => {
    validateFields();
    validateDate();
    if(errors.name || errors.email || errors.lName || errors.phone || errors.between || errors.tooEarly || errors.tooLate) {
...
<div class="booking-container" out:slide>
    ...
    <div class="form-container">
        ...
        <div class="form-element">
            ...
            <input type="text" id="name" placeholder="John" bind:value={bookData.name}/>
        </div>
        <div class="form-element">
            <label for="lName">Last Name</label>
            <input type="text" id="lName" placeholder="Doe" bind:value={bookData.lName} />
        </div>
        <div class="form-element">
            <label for="email">Email</label>
            <input type="email" id="email" placeholder="johndoe@gmail.com" bind:value={bookData.email} />
        </div>
        <div class="form-element">
            <label for="phoneNumber">Phone Number</label>
            <input type="text" id="phoneNumber" placeholder="123-456-7891" bind:value={bookData.phone} />
        </div>    
            <Button on:click={(e) => handleClick(e)} disabled={$bookStore.posting || $bookStore.posted || $bookStore.failed} content="BOOK" primary/>
    </div>
</div>
</Card>
